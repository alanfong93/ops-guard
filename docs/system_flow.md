...
The judge may attach an advisory risk assessment to a proposal, but it does not alter any gate in this flow and cannot authorize execution. Proposal-bound approval is recorded and verified by the internal approval verifier (docs/adr/0003-approval-verifier-boundary.md): the proposing host cannot supply it, and the approval is spent exactly when its token is consumed. Audit events are insert-only and redacted at write time; the pre-execution append commits in the same durable transaction as the token consumption, so a required audit failure refuses the execution.
