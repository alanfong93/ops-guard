...
(the script prints this trace — per-run proposal references are random, the
structure is not; `tests/test_demonstration.py` pins it). The
...
- **audit-write** — a failing execution-start audit append rolls back the
  ...
  (nothing ran, nothing is spent; proven by the gate suite and by the
  demo re-dispatch probe).
...
- On the proposal-bound path, `request.script` is recorded as supplied; the
  MCP integration layer must verify the executed script against it before
  calling the gate.
