...
1. **Canonical invocation identity.** The complete invocation — action, target, typed arguments, preconditions, and runbook revision content hash — is serialized with JCS (RFC 8785) and identified by the SHA-256 digest of those canonical bytes. Any difference in any bound field yields a different identity. Sequence order inside the invocation is significant; callers must sort unordered collections before freezing. Numbers must survive the freeze exactly: an integer argument that would lose precision as an IEEE-754 double is rejected fail-closed, and the frozen bytes must round-trip — re-parsing them must pass the numeric guard and re-canonicalize byte-identically — or the freeze is rejected. Values that cannot satisfy this (for example large powers of two whose ES6 digit-padding re-parses to a non-representable integer) are encoded as strings; freezing them as numbers would produce bytes no consumer could ever re-verify.
...
3. **Opaque one-time token.** Creation mints a 256-bit token from a cryptographically secure random generator and returns it exactly once. Only an HMAC-SHA-256 keyed digest of the token is stored; the raw token is never persisted. The key is an operator-configured secret. The stored digest is exposed read-only on the resolved proposal so approval records (#12) can correlate without touching the raw token.
...
6. **Audit-transaction pairing.** Token consumption and the pre-execution audit append must commit in a single durable transaction. Partial commit — consumed without audit, or audit without consumption — is forbidden. (The audit writer itself is delivered by #9; this contract fixes the pairing requirement.) The callback surface rejects transaction-control SQL — including comment-obscured forms — and attribute reach-ins, and the store refuses to commit a transaction the callback has already ended, raising instead — the guarantee is prevention of accidental transaction control plus detection with fail-closed behaviour, not an in-process sandbox.
